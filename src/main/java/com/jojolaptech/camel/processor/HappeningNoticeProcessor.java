...
/** Step 28d: happening → hrm_company_notice. */
@Component
@RequiredArgsConstructor
public class HappeningNoticeProcessor implements Processor {

    private static final Logger log = LoggerFactory.getLogger(HappeningNoticeProcessor.class);
    ...
    public void process(Exchange exchange) {
        List<Happening> batch = exchange.getMessage().getBody(List.class);
        ...
        Set<Long> mysqlIds = batch.stream()
                .map(row -> MessagingMigrationMapper.HAPPENING_MYSQL_ID_OFFSET + row.getId())
        ...
        for (Happening source : batch) {
            long offsetMysqlId = MessagingMigrationMapper.HAPPENING_MYSQL_ID_OFFSET + source.getId();
            if (existingIds.contains(offsetMysqlId)) {
                continue;
            }
            if (source.getCompany() == null) {
                log.warn("Skipping happening id={}, missing company", source.getId());
                continue;
            }
            CompanyEntity company = companyByMysqlId.get(source.getCompany().getId());
            if (company == null) {
                log.warn(
                        "Skipping happening id={}, company mysqlId={} not migrated",
                        source.getId(),
            ...
            CompanyNoticeEntity mapped =
                    MessagingMigrationMapper.fromHappening(source, company.getId(), employeeIds, publishedBy);
            if (mapped == null) {
                log.warn("Skipping happening id={}, mapping failed", source.getId());
                continue;
            }
            toSave.add(mapped);
