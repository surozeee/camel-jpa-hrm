...
public class TempDeviceLogsProcessor implements Processor {

    private static final Logger log = LoggerFactory.getLogger(TempDeviceLogsProcessor.class);
    ...
    public void process(Exchange exchange) {
        List<TempDeviceLogs> batch = exchange.getMessage().getBody(List.class);
        ...
        int imported = ResidualDeviceAttendanceLogMigrator.migrate(
                batch,
                TempDeviceLogs::getId,
                TempDeviceLogs::getEnrollId,
                TempDeviceLogs::getCheckTime,
                TempDeviceLogs::getCheckType,
                TempDeviceLogs::getMacId,
                TempDeviceLogs::getSensorId,
                TempDeviceLogs::getVerifyCode,
                TempDeviceLogs::getWorkCode,
                AttendancePunchMigrationMapper::tempDeviceLogMysqlId,
                "Migrated from tempDeviceLogs",
                "tempDeviceLogs",
                employeeRepository,
